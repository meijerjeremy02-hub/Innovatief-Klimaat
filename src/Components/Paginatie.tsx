...
export default function Paginatie() {
  ...
  return (
    <div className="lg:fixed lg:top-4 lg:right-0 flex flex-col gap-2 bg-transparent sm:border-2 border-blue-900 rounded-lg p-3">
      <div className="flex gap-1 flex-nowrap items-center justify-center md:pr-15">
        <button
          onClick={() => setHuidig(huidig - 1)}
          disabled={huidig === 0}
          className="px-2 py-1 
          text-sm md:h-10 md:w-10 md:text-lg border-3 bg-white border-blue-900 rounded-md disabled:opacity-30 cursor-pointer"
        >
          ←
        </button>
      ...
      {huidig === TOTAAL - 1 && (
        <button
          ...
          className="w-full md:max-w-[80%] md:pr-1 px-1 text-sm md:mt-1 md:text-lg border-3 border-blue-900 rounded-md bg-orange-200 text-orange-900 cursor-pointer"
        >
          Verstuur ✓
        </button>
      )}
